...
Five locked graphs. Do not add a sixth “experimental” graph. Do not rewrite them from the stock Comfy-Org templates.
...
Default generate is **Ref2VA 8.00 s**. Snap “15 s” / “15.04 s” to `h3-ref2va-long-15s08.json` (**15.08 s / 362**). Never invent **15.00** or **15.04**. Text-only / no identity images uses the FL2VA pair.
...
| Invented 15.00 / 15.04 | Legal **15.08 s / 362** only |

Nodes the Ref2VA graphs must use:
...
- `MiniMaxH3ImageToVideo` — prompt + optional `first_frame` / `last_frame`. Locked T2V JSONs omit those keys today
...
- optional first/last-frame image paths (rejected on today’s locked FL2VA T2V graphs)

Everything else stays locked. 3-view sheets are Ref2VA `--ref-image`, **not** `first_frame`.
